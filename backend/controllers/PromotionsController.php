...
use backend\models\promotions\Promotion;
// use backend\models\cGoal;
// use backend\models\pGoal;
// use backend\models\cGoalsByPromotion;
// use backend\models\pGoalsByPromotion;
...
class PromotionsController extends Controller
{
	...
	public function actionIndex()
	{
		...
		$Data['ModelPromotion']  = $ModelPromotion = Promotion::find();
		$Data['promotionProvider']  = new ActiveDataProvider([
		    'query' => $ModelPromotion,
		    'pagination' => [
		        'pageSize' => 20,
		    ],
		]);
	...
	public function actionPromotion () {
		...
		if (empty($id)) {
			...
			$Promotion->ExpirationDate = date("d/m/Y",strtotime("last day of this month"));
		} else {
			$Promotion = Promotion::findOne($id);
			$Promotion->ExpirationDate = date("d/m/Y",$Promotion->ExpirationDate);
		}
		...
		if ($Promotion->load(Yii::$app->request->post())) {
			...
			if ($Promotion->validate()) {
				...
				try {
					...
					$Date = explode("/", $Promotion->ExpirationDate);
					$Date = $Date[2]."-".$Date[1]."-".$Date[0];

					$Promotion->ExpirationDate = date("U",strtotime($Date));
